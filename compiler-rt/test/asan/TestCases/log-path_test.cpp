...
// A filename exceeding NAME_MAX cannot be opened, even when running as root.
// RUN: %python -c "print('X' * 256, end='')" > %t.invalid_log_path
// RUN: %env_asan_opts=log_path=INVALID%{readfile:%t.invalid_log_path} not %run %t 2> %t.out
